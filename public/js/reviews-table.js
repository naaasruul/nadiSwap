...
if (document.getElementById("reviews-table") && typeof simpleDatatables.DataTable !== 'undefined') {
    ...
    const table = new simpleDatatables.DataTable("#reviews-table", {
        labels: {
            placeholder: "Search reviews...",
            searchTitle: "Search within reviews",
            pageTitle: "Page {page}",
            perPage: "Reviews per page",
            noRows: "No reviews found",
            info: "Showing {start} to {end} of {rows} reviews",
            noResults: "No reviews match your search query",
        },
        columns: [
            { 
                select: 0, 
    ...
    table.on("datatable.init", () => {
        console.log("Reviews DataTable initialized");
        
        // Add custom styling to the table
        const tableElement = document.querySelector("#reviews-table");
        if (tableElement) {
    ...
    table.on("datatable.refresh", () => {
        console.log("Reviews DataTable refreshed");
        setTimeout(() => {
            initializeDropdown();
        }, 100);
    });
    ...
    window.refreshReviewsTable = () => {
        if (table) {
            table.refresh();
        }
    };

    console.log("Reviews DataTable script loaded successfully");
}