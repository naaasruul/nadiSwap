					...
					<form action="{{ route('contact.submit') }}" method="POST" class="space-y-6">
						...
						<div>
							...
							<input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="John Doe"
								class="block w-full px-4 py-3 border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg focus:ring-pink-500 focus:border-pink-500"
								required>
							@error('name') <p class="text-pink-500 text-sm mt-1">{{ $message }}</p> @enderror
						</div>
						...
						<div>
							...
							<input type="email" id="email" name="email" value="{{ old('email') }}" placeholder="Email"
								class="block w-full px-4 py-3 border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg focus:ring-pink-500 focus:border-pink-500"
								required>
							@error('email') <p class="text-pink-500 text-sm mt-1">{{ $message }}</p> @enderror
						</div>
						...
						<div>
							...
							<input type="text" id="subject" name="subject" value="{{ old('subject') }}" placeholder="Subject"
								class="block w-full px-4 py-3 border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-whitWe rounded-lg focus:ring-pink-500 focus:border-pink-500"
								required>
							@error('subject') <p class="text-pink-500 text-sm mt-1">{{ $message }}</p> @enderror
						</div>
						...
						<div>
							...
							@error('message') <p class="text-pink-500 text-sm mt-1">{{ $message }}</p> @enderror
						</div>

						<!-- Submit Button -->
						<button type="submit"
							class="cursor-pointer inline-block w-full sm:w-auto px-6 py-3 text-sm font-semibold text-white bg-pink-600 rounded-lg hover:bg-pink-700 focus:outline-none focus:ring-2 focus:ring-pink-400">
							Send Message
						</button>
					</form>
				</div>
