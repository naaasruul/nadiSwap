@props(['orderId', 'date', 'price', 'delivery_status', 'payment_status', 'payment_method', 'order_status', 'actions' => []])
...
<div class="flex flex-wrap items-center gap-y-4 border-b border-gray-200 pb-4 dark:border-gray-700 md:pb-5">
    ...
    <dl class="w-1/2 sm:w-1/5 md:flex-1 lg:w-auto">
        ...
        <dd class="mt-1.5 text-base font-semibold text-gray-900 dark:text-white">RM {{ (number_format($price, 2)) }}</dd>
    </dl>
    ...
    @php
        $deliveryClasses = match ($delivery_status) {
    ...
    <dl class="w-1/2 sm:w-1/4 sm:flex-1 lg:w-auto">
        ...
        {{ $deliveryClasses }}
         px-2.5 py-0.5 text-xs font-medium">
            ...
            {{ $delivery_status == 'ofd' ? 'Out for Delivery' : ucfirst($delivery_status) }}
        </dd>
    </dl>

    @php
        $paymentClasses = match ($payment_status) {
    ...
    @php
        $orderClasses = match ($order_status) {
            'pending' => 'bg-yellow-100 text-yellow-800',
    ...
    @endphp
    <dl class="w-1/2 sm:w-1/4 sm:flex-1 lg:w-auto">
        <dt class="text-base font-medium text-gray-500 dark:text-gray-400">Order Status:</dt>
        <dd class="me-2 mt-1.5 inline-flex shrink-0 items-center rounded  capitalize
        {{ $orderClasses }}
         px-2.5 py-0.5 text-xs font-medium">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="me-1 h-3 w-3"
