@props(['orderId', 'date', 'price', 'status', 'actions' => []])
...
<div class="flex flex-wrap items-center gap-y-4 border-b border-gray-200 pb-4 dark:border-gray-700 md:pb-5">
    <dl class="w-1/2 sm:w-48">
        ...
        <dd class="mt-1.5 text-base font-semibold text-gray-900 dark:text-white">
            <a href="#" class="hover:underline">{{ $orderId }}</a>
        </dd>
    </dl>
    ...
    <dl class="w-1/2 sm:w-1/4 sm:flex-1 lg:w-auto">
        ...
        {{ $status == 'completed' ? 'bg-green-100 text-green-800' : ($status == 'pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800') }}
    ...
    {{-- <div class="w-full sm:flex sm:w-32 sm:items-center sm:justify-end sm:gap-4">
        <button
            id="actionsMenuDropdownModal{{ $orderId }}"
            data-dropdown-toggle="dropdownOrderModal{{ $orderId }}"
            type="button"
        ...
        <div id="dropdownOrderModal{{ $orderId }}" class="z-10 hidden w-40 divide-y divide-gray-100 rounded-lg bg-white shadow dark:bg-gray-700" data-popper-reference-hidden="" data-popper-escaped="" data-popper-placement="bottom">
            <ul class="p-2 text-left text-sm font-medium text-gray-500 dark:text-gray-400" aria-labelledby="actionsMenuDropdown{{ $orderId }}">
